...
void PS()
{
	...
	vec4 adv =  texture2D(sDetailMap1,vTexCoord);
	vec4 stw =  texture2D(sDiffMap,vTexCoord);

	vec4 inp =  texture2D(sDetailMap2,vTexCoord);///to test
	//vec3 
	//gl_FragColor=vec4(m,1.0);

	#ifdef LEGACY

		m =  texture2D(sDetailMap2,vTexCoord).xyz;
	...
	//gl_FragColor=vec4(vec3(adv.a),1.0);
	//gl_FragColor=vec4(vec3(inp.b),1.0);
}