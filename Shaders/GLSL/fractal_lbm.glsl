...
void PS(){
     #ifdef  LBM
        //red and green are random vectors and velocity to start
        //blue channel is advection rate or life, or something to that effect. a control.
        float red = bias(fbm(((vScreenPos.xyz+vec3(20.0,-1.0,42.44))*vec3(1.5,1.5,1.5))+(cElapsedTimePS*0.08)),0.01)*5.0;
        float green = bias(fbm(((vScreenPos.xyz+vec3(-11.1,13.3,2.01))*vec3(1.5,1.5,1.5))+(cElapsedTimePS*0.08)),0.01)*5.0;
        float blue = bias( fbm((vScreenPos.xyz*vec3(0.5,0.5,0.5))+(cElapsedTimePS*0.12)),0.01 )*5.0;
